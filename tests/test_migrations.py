...
class MigrationTests(unittest.TestCase):
    def test_new_database_creates_baseline_schema(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(db.migrate(),[1,2,3,4,5,6,7])
            with db.connect() as connection:
                tables={row[0] for row in connection.execute(
                    "SELECT name FROM sqlite_master WHERE type='table'"
                )}
            self.assertTrue({"jobs","candidates","errors","snapshots","schema_migrations","keepa_cache","keepa_usage","keepa_cache_hits","opportunities","opportunity_signals","virtual_purchases","virtual_purchase_observations","seller_monitors","seller_monitor_asins","seller_monitor_detections"} <= tables)
    ...
    def test_v1_database_upgrades_to_latest_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path)
            self.assertEqual(db.migrate(),[2,3,4,5,6,7])
            with db.connect() as connection:
                self.assertEqual(connection.execute("SELECT COUNT(*) FROM jobs").fetchone()[0],1)
    ...
    def test_v2_database_upgrades_to_v3_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(db.migrate(),[3,4,5,6,7])
            self.assertEqual(db.schema_version(),7)
            with db.connect() as connection:
    ...
    def test_v3_database_upgrades_to_v4_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path)
            self.assertEqual(db.migrate(),[4,5,6,7]); self.assertEqual(db.schema_version(),7)
            with db.connect() as connection:
    ...
    def test_v4_database_upgrades_to_v5_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path); self.assertEqual(db.migrate(),[5,6,7]); self.assertEqual(db.schema_version(),7)
            with db.connect() as connection:
    ...
    def test_v5_database_baselines_existing_virtual_purchase_as_default_estimate(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path); self.assertEqual(db.migrate(),[6,7]); self.assertEqual(db.schema_version(),7)
            with db.connect() as connection:
