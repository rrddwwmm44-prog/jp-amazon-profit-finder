...
class MigrationTests(unittest.TestCase):
    def test_new_database_creates_baseline_schema(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(db.migrate(),[1,2,3,4,5,6,7,8,9])
    ...
    def test_v1_database_upgrades_to_latest_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path)
            self.assertEqual(db.migrate(),[2,3,4,5,6,7,8,9])
            with db.connect() as connection:
                self.assertEqual(connection.execute("SELECT COUNT(*) FROM jobs").fetchone()[0],1)
    ...
    def test_v2_database_upgrades_to_v3_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertEqual(db.migrate(),[3,4,5,6,7,8,9])
            self.assertEqual(db.schema_version(),9)
            with db.connect() as connection:
    ...
    def test_v3_database_upgrades_to_v4_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path)
            self.assertEqual(db.migrate(),[4,5,6,7,8,9]); self.assertEqual(db.schema_version(),9)
            with db.connect() as connection:
    ...
    def test_v4_database_upgrades_to_v5_without_data_loss(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path); self.assertEqual(db.migrate(),[5,6,7,8,9]); self.assertEqual(db.schema_version(),9)
            with db.connect() as connection:
    ...
    def test_v5_database_baselines_existing_virtual_purchase_as_default_estimate(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path); self.assertEqual(db.migrate(),[6,7,8,9]); self.assertEqual(db.schema_version(),9)
            with db.connect() as connection:
    ...
    def test_v7_virtual_purchase_is_safely_baselined_for_comparison(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            connection.commit(); connection.close()
            db=Database(path); self.assertEqual(db.migrate(),[8,9])
            with db.connect() as connection:
    ...
    def test_v8_detection_is_pending_after_processing_migration(self):
        with tempfile.TemporaryDirectory() as raw:
            path=Path(raw)/"v8.db"; connection=sqlite3.connect(path)
            connection.execute(MIGRATION_TABLE_SQL)
            for migration in MIGRATIONS[:8]:
                for statement in migration.statements: connection.execute(statement)
                connection.execute("INSERT INTO schema_migrations(version,name) VALUES(?,?)",(migration.version,migration.name))
            connection.execute("INSERT INTO seller_monitors(seller_id,created_at,updated_at) VALUES('A12345678901','2026-01-01','2026-01-01')")
            connection.execute("INSERT INTO seller_monitor_detections(asin,seller_id,detected_at) VALUES('B000000001','A12345678901','2026-01-02')")
            connection.commit(); connection.close()
            db=Database(path); self.assertEqual(db.migrate(),[9])
            with db.connect() as connection:
                row=connection.execute("SELECT processing_status,processed_at,processing_error FROM seller_monitor_detections").fetchone()
