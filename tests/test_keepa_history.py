...
def raw_product() -> dict:
    ...
    return {"asin": ASIN, "title": "History fixture", "domainId": 5, "availabilityAmazon": 0, "csv": csv}
...
class KeepaHistoryTests(unittest.TestCase):
    ...
    def test_invalid_minus_one_is_excluded(self):
        points = decode_history([keepa_time(NOW), -1, keepa_time(NOW), 10])
        self.assertEqual([point.value for point in points], [10])
    ...
    def test_30_and_90_day_medians_require_minimum_samples(self):
        ...
        self.assertEqual(window_median(points, NOW, 30, 3), 5000.0)
        self.assertEqual(window_median(points, NOW, 90, 3), 5500.0)
        self.assertIsNone(window_median(points, NOW, 30, 4))
    ...
    def test_arbitrage_adapter_uses_amazon_purchase_price(self):
        ...
        self.assertEqual(item.median_price_30d, 5550.0)
